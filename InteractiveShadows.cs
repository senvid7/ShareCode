...
public class InteractiveShadows : MonoBehaviour
{
    ...
    private Vector3 ComputeIntersectionPoint(Vector3 fromPosition, Vector3 direction)
    {
        ...
        else if (Physics.Raycast(fromPosition, direction, out hit, Mathf.Infinity, groundLayerMask))
        {
            if (Physics.Raycast(hit.point, Vector3.ProjectOnPlane(direction, new Vector3(0,1,0)), out hit, Mathf.Infinity, targetLayerMask))
            {
                return hit.point - transform.position;
            }
        }
